...
class SalarySlipMailerApp:
    def __init__(self, root):
        ...
        self.settings_menu.add_command(label="Email Settings", command=self.open_settings_window)
        ...
        self.email_body = ""  # Initialize email body as an empty string

    def open_settings_window(self):
        """Open the Email Settings Window."""
        self.settings_window = EmailSettingsWindow(self.root)
        self.root.wait_window(self.settings_window.top)  # Wait for settings window to close

        # After settings window closes, retrieve the email body
        self.email_body = self.settings_window.get_email_body()

        if not self.email_body:
            logging.warning("No email body provided. Please check your settings.")
    ...
    # Cat phieu luong theo ma nhan vien
        ...
        try:
            ...
            for index, row in data.iterrows():
                try:
                    ...
                    #pdf.set_font("DejaVu", style="B", size=12)  # Bold font

                    # Add bold, red text
                    pdf.cell(100, 5, txt=f"{int(total06):,}", ln=True, align='R')
                    pdf.cell(100, 5, txt=f"19 Làm HC ngày lễ (300%): {int(total24):,}", align='R',ln=True)
    ...
    # bat dau gui email khi user an nut Send Email
    ...
    def start_sending_emails(self):
        ...
        email_thread = threading.Thread(target=self.run_email_process, args=(excel_file, folder_path))
        email_thread.daemon = True  # Ensures thread won't block program exit
        email_thread.start()
    ...
    def load_email_config(self, config_path="email_config.json"):
        ...
        try:
            ...
            raise Exception(f"Configuration file '{config_path}' is not a valid JSON file.")
    ...
    def run_email_process(self, excel_file, folder_path):
        ...
        try:
            ...
            body_template = config.get("default_body", "Default Subject")

            # Load Excel data
            xls = pd.ExcelFile(excel_file)
            xls1 = pd.ExcelFile("EMAIL.xls")

            if 'Salary Sheet' not in xls.sheet_names:
                raise ValueError("Worksheet named 'Salary Sheet' not found.")

            # Parse data
            data1 = xls1.parse(sheet_name='Export Data').iloc[8:]
            ...
            for index, row in data1.iterrows():
                ...
                if pd.isna(email) or not email:
                    logging.warning(f"Row {index}, Staff ID {staff_id}, Name {name}: Missing email.")
                    continue

                # Format email subject and body
                subject = subject_template.format(name=name, staff_id=staff_id)
                body = body_template.format(name=name, staff_id=staff_id)

                # Check for attachment
                attachment_path = os.path.join(folder_path, f"{staff_id}_Salary_Slip.pdf")
                if not os.path.exists(attachment_path):
                    logging.warning(f"Attachment not found for Staff ID {staff_id} ({name}). Skipping email.")
                    continue

                # Send email
                if not self.send_email(email, subject, body, attachment_path):
                    logging.error(f"Failed to send email for row {index}.")
                else:
                    logging.info(f"Email successfully sent to {email}.")

            self.status_label.config(text="Status: Emails sent successfully!", fg="blue")
        except Exception as e:
            self.status_label.config(text="Status: Failed to send emails.", fg="red")
            messagebox.showerror("Error", f"An error occurred: {e}")

    def send_email(self, email, subject, body, attachment_path):
        """
        Send an email with the given subject, body, and attachment.
        """
        ...
        try:
            ...
            if not sender_email or not password:
                raise ValueError("Email configuration is incomplete. Check email_config.json.")

            # Prepare the email message
            msg = MIMEMultipart()
            msg['From'] = sender_email
            msg['To'] = email
            msg['Subject'] = subject  # Use subject passed from run_email_process
            msg.attach(MIMEText(body, 'plain'))  # Use body passed from run_email_process
    ...
    def handle_email_data(subject, body):
...
class EmailSettingsWindow:
    def __init__(self, master):
        ...
        self.test_recipient_label = tk.Label(self.top, text="Test Recipient Email:")
        self.test_recipient_label.grid(row=5, column=0, padx=10, pady=5, sticky="e")
        self.test_recipient_entry = tk.Entry(self.top, width=40)
        self.test_recipient_entry.grid(row=5, column=1, padx=10, pady=5, sticky="w")

        # Email Body Input
        self.body_label = tk.Label(self.top, text="Email Body:")
        self.body_label.grid(row=6, column=0, padx=10, pady=5, sticky="ne")
        self.body_text = tk.Text(self.top, height=10, width=30)
        self.body_text.grid(row=6, column=1, padx=10, pady=5, sticky="w")
        self.body_text.insert("1.0", self.settings.get("default_body", ""))
        ...
        self.test_button.grid(row=8, column=0, columnspan=2, pady=10, sticky="ew")

    def create_input_fields(self):
        # Sender Email
        self.sender_label = tk.Label(self.top, text="Sender Email:")
        self.sender_label.grid(row=0, column=0, padx=10, pady=5, sticky="e")
        self.sender_entry = tk.Entry(self.top, width=40)
        self.sender_entry.grid(row=0, column=1, padx=10, pady=5, sticky="w")
        ...
        self.password_entry = tk.Entry(self.top, width=40, show="*")
        ...
        self.smtp_label.grid(row=2, column=0, padx=10, pady=5, sticky="e")
        self.smtp_entry = tk.Entry(self.top, width=40)
        self.smtp_entry.grid(row=2, column=1, padx=10, pady=5, sticky="w")
        self.smtp_entry.insert(0, self.settings.get("smtp_server", "smtp.gmail.com"))

        # Port
        self.port_label = tk.Label(self.top, text="Port:")
        self.port_label.grid(row=3, column=0, padx=10, pady=5, sticky="e")
        self.port_entry = tk.Entry(self.top, width=40)
        self.port_entry.grid(row=3, column=1, padx=10, pady=5, sticky="w")
        self.port_entry.insert(0, self.settings.get("port", "587"))

        # Subject
        self.subject_label = tk.Label(self.top, text="Subject:")
        self.subject_label.grid(row=4, column=0, padx=10, pady=5, sticky="e")
        self.subject_entry = tk.Entry(self.top, width=40)
        self.subject_entry.grid(row=4, column=1, padx=10, pady=5, sticky="w")
    ...
    def save_settings(self):
        try:
            # Validate email
            if not re.match(r"[^@]+@[^@]+\.[^@]+", self.sender_entry.get()):
                messagebox.showerror("Error", "Invalid sender email address.")
                return

            # Validate port
            if not self.port_entry.get().isdigit():
            ...
            self.settings = {
                ...
                "default_body": self.body_text.get("1.0", "end").strip(),
            }
    ...
    def get_subject(self):
        """Return the subject entered by the user."""
    ...
    def done(self):
        """Pass the subject and body back to the main app."""
        if self.callback:
            subject = self.get_subject()
            body = self.get_body_text()
            self.callback(subject, body)
        self.top.destroy()
    ...
    def send_test_email(self):
        try:
            ...
            recipient_email = self.test_recipient_entry.get()
