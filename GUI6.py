...
class SalarySlipMailerApp:
    ...
    def run_email_process(self, excel_file, folder_path):
        ...
        for index, row in data.iterrows():
            ...
            if not os.path.exists(attachment_path):
                logging.warning(f"Attachment not found for Staff ID {staff_id}. Skipping email.")
                continue
    ...
    def send_email(self, email, subject, body, attachment_path):
        ...
        try:
            ...
            sender_email = config.sender_entry.get()
            password = config.password_entry.get()
            smtp_server = config.smtp_entry.get()
            port = int(config.port_entry.get())
...
class EmailSettingsWindow:
    ...
    def create_input_fields(self):
        ...
        self.test_recipient_label.pack(pady=5)
        self.test_recipient_entry = tk.Entry(self.top, width=30)
        self.test_recipient_entry.pack(pady=5)
