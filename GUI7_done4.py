    ...
    def run_email_process(self, excel_file, folder_path):
        ...
        try:
            ...
            xls1 = pd.ExcelFile("EMAIL.xls")
             ...
             # In danh sách các sheet có trong file Excel
            #print(f"Sheet names: {xls.sheet_names}")
            
            # Kiểm tra nếu sheet 'Salary Sheet' có tồn tại
            if 'Salary Sheet' not in xls.sheet_names:
                  raise ValueError("Worksheet named 'Salary Sheet' not found.")
            
            # Lấy dữ liệu từ sheet 'Salary Sheet'
            data = xls.parse(sheet_name='Salary Sheet')
            data = data.iloc[10:]
            data1 = xls1.parse(sheet_name='Export Data')
            data1 = data1.iloc[9:]
            #print(data1)
            ...
            # Đổi tên cột cho dễ đọc, giả sử các cột bạn cần là 'Họ tên' và 'Mã nhân viên'
            data1.rename(columns={
                  'Unnamed: 2': 'Name',  # Thay 'Họ tên' thành 'Name'
                  'Unnamed: 33': 'Email',  # Thay 'Mã nhân viên' thành 'Staff ID'
                  'Unnamed: 1': 'Staff ID'
            }, inplace=True)
            
            # Nếu email không có sẵn trong file, bạn có thể thêm một cột email ở đây
            # Ví dụ: data['Email'] = 'email@example.com' (Chú ý thay bằng dữ liệu thực tế của bạn)
            ...
            for index, row in data1.iterrows():
                ...
                if pd.isna(email) or not email:
                    logging.warning(f"Dong:{index},Nhan vien:{staff_id},Ho&ten:{name}: Error email")
                    logging.info(f"Dong:{index},Nhan vien:{staff_id},Ho&ten:{name}: Error email")
                    continue

                subject = "Noreply_PHIẾU LƯƠNG/THƯỞNG_CTY TNHH MARUICHI SUN STEEL (HÀ NỘI)"
                body = (
                    f"Xin Chào {name}!\n\n"
...
class EmailSettingsWindow:
    ...
    def create_input_fields(self):
        ...
        # Test Recipient Email and Button
        self.test_recipient_label = tk.Label(self.top, text="Test Email:")
        self.test_recipient_label.grid(row=4, column=0, padx=10, pady=5, sticky="e")
        self.test_recipient_entry = tk.Entry(self.top, width=30)
        self.test_recipient_entry.grid(row=4, column=1, padx=10, pady=5, sticky="w")

        self.test_button = tk.Button(self.top, text="Send", command=self.send_test_email)
        self.test_button.grid(row=4, column=2, padx=10, pady=5)

    def load_settings(self):
        # Stub for loading settings (replace with your implementation)
        return {}

    def save_settings(self):
        # Stub for saving settings (replace with your implementation)
        pass

    def send_test_email(self):
        # Stub for sending test email (replace with your implementation)
        pass   
    ...
    def send_test_email(self):
        try:
            ...
            with smtplib.SMTP(smtp_server, port) as server:
                ...
                message["Subject"] = "MARUICHI SUN STEEL (HA NOI) Co.,Ltd"
